...
namespace AutoImporterForAseprite
{
    public class AsepriteCli
    {
        ...
        public void ExportSpriteSheet(string aseFile, string sheetOutPath, string sheetDataOutPath, int scale = -1)
        {
            ...
            if (sheetOutPath.EndsWith(".png") == false)
                sheetOutPath += ".png";


            if (sheetDataOutPath.EndsWith(".json") == false)
            ...
            argumentsSb.Append($" {aseFile}");
            argumentsSb.Append($" --scale {scale}");
            ...
            if (!string.IsNullOrEmpty(importOptions.fileNameFormat))
            {
                ...
                argumentsSb.Append($" --filename-format {ff}");
            }

            argumentsSb.Append($" --list-tags --sheet {sheetOutPath}");
            argumentsSb.Append($" --format json-array --data {sheetDataOutPath}");
