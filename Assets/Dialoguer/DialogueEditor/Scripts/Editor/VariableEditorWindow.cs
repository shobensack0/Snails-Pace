...
public class VariableEditorWindow : EditorWindow {
	...
	void OnGUI(){
		...
		Rect titleRect = new Rect(5,5,position.width-10, 22);
		if(isPro){
		...
		GUI.Label(new Rect(titleRect.x + 5, titleRect.y + 3, position.width, 20), titleText, EditorStyles.boldLabel);
		
		if(GUI.Toggle(new Rect(5,30,(position.width - 10)*0.5f, 25), (__scope == VariableEditorScopes.Global), "Globals ("+globalVarsCount+")", DialogueEditorGUI.gui.GetStyle("toolbar_left"))) setTarget(VariableEditorScopes.Global, __type);
		if(GUI.Toggle(new Rect((position.width - 20)*0.5f + 10,30,(position.width - 10)*0.5f, 25), (__scope == VariableEditorScopes.Local), "Locals ("+localVarsCount+")", DialogueEditorGUI.gui.GetStyle("toolbar_right"))) setTarget(VariableEditorScopes.Local, __type);

		Rect typesRect = new Rect(5, 60, position.width - 10, 25);
		...
		// Editor Box
		Rect editorRect = new Rect(5, typeBooleanToggleRect.y + typeBooleanToggleRect.height + 5, position.width - 10, 130);
		drawEditorGui(editorRect);
		
		// ------------------ SCROLL BOX
		// VISUALS
		Rect scrollRect = new Rect(8,editorRect.y + editorRect.height + 8, position.width - 16, position.height - (editorRect.y + editorRect.height + 70));
		if(isPro){
		...
		// ADD/REMOVE BUTTONS
		//ADD
		Rect addButtonRect = new Rect(5,scrollRect.y + scrollRect.height + 8,(position.width - 10)*0.5f, 25);
		if(GUI.Button(addButtonRect, "Add", DialogueEditorGUI.gui.GetStyle("toolbar_left"))){
			variables.addVariable();
		}
		
		//REMOVE
		Rect removeButtonRect = new Rect((position.width - 10)*0.5f + 5,scrollRect.y + scrollRect.height + 8,(position.width - 10)*0.5f, 25);
